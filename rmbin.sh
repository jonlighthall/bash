...
then
	...
	if git rev-parse --git-dir > /dev/null 2>&1; then
	    ...
	    find ./ -type f -not -path "*$GITDIR/*" | perl -lne 'print if -B' | xargs -r git rm --ignore-unmatch -n

	    # then, remove remaining binary files
	    echo "removing untracked binary files..."
	    find ./ -type f -not -path "*$GITDIR/*" | perl -lne 'print if -B' | xargs -r ls
	else
	    # this is not a git repository
	    echo "$1 is not part of a Git repsoity"
	    echo "removing binary files..."
	    find ./ -type f | perl -lne 'print if -B' | xargs -r ls	    
	fi
    else
	echo "$1 is not found"
	exit 1
